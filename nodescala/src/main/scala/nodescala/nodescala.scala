...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    async {
      while(token.cancellationToken.nonCancelled){
        val req = await(listener.nextRequest)
        val procReq = Future.run() {
          ct => Future{
            while(ct.nonCancelled)
              respond(req._2, token.cancellationToken, handler(req._1))
          }
        }
        
        Future.delay(5 seconds) onSuccess {
          case x => procReq.unsubscribe()
          
        }
      }
    }
    
    Subscription(listenerSubscription, token)
  }
}
