...
trait WikipediaApi {
  ...
  implicit class StringObservableOps(obs: Observable[String]) {
     ...
     */
    def sanitized: Observable[String] = obs flatMap {
      s => Observable.from(Future(s.replaceAll(" ", "_")))
    }
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
     ...
     */
    def recovered: Observable[Try[T]] = obs map (x => Try(x)) onErrorReturn { er => Failure(er) }
    ...
    def timedOut(totalSec: Long): Observable[T] = {
      val timeOut = Observable.interval(totalSec seconds).take(1)
     ...
     */
    def concatRecovered[S](requestMethod: T => Observable[S]): Observable[Try[S]] = obs.concatMap { x => requestMethod(x).recovered }
