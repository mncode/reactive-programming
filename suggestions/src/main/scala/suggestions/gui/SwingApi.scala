...
trait SwingApi {
  ...
  implicit class TextFieldOps(field: TextField) {
    ...
    def textValues: Observable[String] = Observable.create[String](observer => {
      ...
      if (subscription.isUnsubscribed) {
        ...
        try {
          field.subscribe(x => x match {
            case ValueChanged(e) => observer.onNext(e.text)
          })
  ...
  implicit class ButtonOps(button: Button) {
     ...
     */
    def clicks: Observable[Button] = Observable.create[Button](observer => {
      val subscription = Subscription()
      ...
      if (subscription.isUnsubscribed) {
        ...
        try {
          button.subscribe(x => x match {
            case ButtonClicked(e) => observer.onNext(e)
          })
    ...
    /*val subscription = Subscription()
        Future {
          try {
            ...
            case error: Exception => observer.onError(error)
          }
        }
