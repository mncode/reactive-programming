...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    val suggestions: Observable[Try[List[String]]] = searchTerms.concatRecovered {
      x => Observable.from(wikipediaSuggestion(x))
    }

    // TO IMPLEMENT
    val suggestionSubscription: Subscription = suggestions.observeOn(eventScheduler) subscribe (
      value => {
        val prev = suggestionList.listData
        //logger.info(s"suggestion list $prev")
        suggestionList.listData = value.get
      },
      cause => {
        status.text = (" No suggestion !")
      },
      () => ())

    // TO IMPLEMENT
    val selections: Observable[String] = Observable.create[String](observer => {
      ...
      if (subscription.isUnsubscribed) {
        ...
        try {
          button.subscribe(x => x match {
            case ButtonClicked(e) => observer.onNext(suggestionList.selection.items.head)
          })
    ...
    val pages: Observable[Try[String]] = selections.concatRecovered {
      term => Observable.from(wikipediaPage(term))
    }
    ...
    val pageSubscription: Subscription = pages.observeOn(eventScheduler) subscribe (
      value => value match {
        case Success(text) => {
          //logger.info(s"page subscription $text")
          editorpane.text = text
        }
        case Failure(ex) => status.text = "Page not found"
      },
      cause => {
        //logger.info(s" page subscritpion failed ${cause.getCause()}")
        status.text = "Page not found yet"
      },
      () => ())
